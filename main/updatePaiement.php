...
require_once ('connect.php');
	...
	$id = $_GET['cin'];
	$selSql = "SELECT * FROM `employe` WHERE cin=$cin";
...
<div class="form-group" id="postesContainer">
    ...
    <div class="col-sm-10">
        <select class="form-control" id="inputPoste" name="poste">
            <?php
            // Assume $posteValues is an array containing values from the database
            foreach ($posteValues as $posteValue) {
                // Output option with value from the database
                echo '<option value="' . $posteValue . '"';
                // Check if the default poste value matches the current poste value
                if ($defaultPoste == $posteValue) {
                    echo ' selected'; // Set as selected if matches
                }
...
$(document).ready(function() {
    // Function to populate the Poste dropdown based on the selected department
    function populatePosteDropdown(selectedDepartment) {
        var postes = [];
        // Clear existing options
        $('#inputPoste').empty();
        // Add options based on selected department
